...
% Добавление дополнительного уровня \subsubsubsection
...
Прототипирование должно ответить на вопрос: возможно ли, в принципе, сконструировать плавательное средство с высоким центром тяжести, устойчивое относительно бортовой качки, и опробовать технологии и алгоритмы для создания полноразмерной доски.

Чтобы построение прототипа было осмысленным, а результат был неслучайным, в первую очередь необходимо понять с чем мы работаем и в чем состоит задача с математической точки зрения. В этом помогают две схемы (рис.~$\ref{fig:typical_auto_control}$~и~$\ref{fig:my_process}$).
...
\begin{figure}[h]
	\begin{minipage}[h]{0.49\linewidth}
		\center{\includegraphics[width=0.7\linewidth]{images/shtanga.png} \\ штанга}
	\end{minipage}
	\hfill
	\begin{minipage}[h]{0.49\linewidth}
		\center{\includegraphics[width=0.75\linewidth]{images/kil.png} \\ киль}
	\end{minipage}
...
\begin{figure}
	\center{\includegraphics[width=1\linewidth]{images/ext_kalman.png}}
...
\subsubsubsection{Этап экстраполяции}
...
\subsubsubsection{Этап коррекции}
...
\subsubsubsection{Неполные измерения}
...
\subsubsubsection{Получение ориентации}
...
\begin{figure}
	...
	\centering
	\begin{tikzpicture}
		\draw[thick] (-\R,0) arc (180:360:\R cm);
		\node[below left] at (-\R,0) {$-R$};
		\node[below right] at (\R,0) {$R$};

		\draw[thick] (-\R,0) -- (\R,0);
		\draw[thick] (-\w/2,0) rectangle (\w/2,\h);
		\node[above right] at (0,\h) {$h$};
		\node[below] at (-\w/2,0) {$-w/2$};
		\node[below] at (\w/2,0) {$w/2$};

		\draw[thick,snake=coil, segment aspect=0] (-\R-1,-\d) -- (\R+1,-\d);
		\node[below right] at (0,-\d) {$d$};
		%\draw[step=1cm,gray,very thin] (-3.5,-3.9) grid (3.5,5.9);
		\draw[thick,->] (-\R-1,0) -- (\R+1,0) node[anchor=north west] {$x$};
		\draw[thick,->] (0,-\R-1) -- (0,\h+1) node[anchor=south east] {$y$};
...
Мы рассмотрим фактически двумерную модель доски, основанную на законе изменения момента импульса. Для более сложных моделей обратитесь к \cite{fossen_marine_control}.
...
Забежим немного вперед и посмотрим, что произойдет с доской при отклонении от вертикали на угол $\alpha$ (см.~рис.~$\ref{fig:incline}$). Ось бортовой качки для кораблей и лодок проходит через ватерлинию. Соответственно, точка на переcечении пунктирных линий и есть ось вращения при колебаниях с малой амплитудой. Из рисунка видно, что при наклоне, сила тяжести, приложенная в центре масс, создает опрокидываюший момент, а сила Архимеда, приложенная в центре плавучести, создает возвращающий момент.
...
\begin{figure}
	...
	\def\d{1}
	\def\a{15}
	\def\c{2.5} % высота центра масс над лодкой
	\def\bx{0.2} % центр плавучести
	\def\by{-1.8}

	\centering
	\begin{tikzpicture}[scale=0.6]
		% лодка с грузом
		\draw[thick, rotate=-\a, anchor=north] (-\R,0) arc (180:360:\R cm);
		\draw[thick, rotate=-\a, anchor=north] (-\R,0) -- (\R,0);
		\draw[thick, rotate=-\a, anchor=north] (-\w/2,0) rectangle (\w/2, \h);

		% пунктиры для оси вращения
		\draw[thin, dashed, rotate=-15, anchor=north] (-\R-1,-\d) -- (\R+1,-\d); 
		\draw[thin, dashed, rotate=-15, anchor=north] (0,-\R-1) -- (0,\h+1);

		% вода
		\draw[thin,snake=coil, segment aspect=0] (-\R-1,-\d) -- (\R+1,-\d);

		% вертикальная ось
		\draw[thin] ({\d*sin(-\a)},{-\d*cos(\a)}) -- ({\d*sin(-\a)},\h+1);

		% сила тяжести
		\fill[black] ({-\c*sin(-\a)}, {\c*cos(\a)}) circle (3pt);
		\draw[thick, ->] ({-\c*sin(-\a)}, {\c*cos(\a)}) -- node [right, pos=0.95] {$\vec{mg}$} ({-\c*sin(-\a)}, -\R-1.5); 

		% сила Архимеда
		\fill[black] (\bx, \by) circle (3pt);
		\draw[thick, ->] (\bx, \by) -- node [right, pos=0.95] {$\vec F_A$} (\bx, \h);

		% подписи
		\node (buyo_label) at (-\R*1.5,-\R*1.5) {Центр плавучести};
		\draw[very thin, ->] (buyo_label) -- (\bx,\by);

		\node (mc_label) at (\R*2,\h) {Центр масс};
		\draw[very thin, ->] (mc_label) -- ({-\c*sin(-\a)}, {\c*cos(\a)});

	\end{tikzpicture}
	\caption{Что происходит при наклоне доски}	
	\label{fig:incline}
\end{figure}
...
\subsubsubsection{Глубина погружения}
...
\subsubsubsection{Площадь подводной части}

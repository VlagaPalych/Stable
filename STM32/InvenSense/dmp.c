...
int DMP_LoadMotionDriverFirmware(void) {
    return MPU_LoadFirmware(DMP_CODE_SIZE, dmp_memory, sStartAddress,
        DMP_SAMPLE_RATE);
}
...
int DMP_SetFIFORate(uint16_t rate) {
    ...
    if (MPU_MemWrite(D_0_22, tmp, 2))
        return -1;
    if (MPU_MemWrite(CFG_6, (uint8_t *)regs_end, 12))
        return -1;
...
int DMP_EnableFeature(uint16_t mask) {
    ...
    if (mask & DMP_FEATURE_6X_LP_QUAT)
        DMP_Enable6XLPQuat(1);
    else
        DMP_Enable6XLPQuat(0);
...
int DMP_EnableGyroCal(uint8_t enable) {
    if (enable) {
        ...
        return MPU_MemWrite(CFG_MOTION_BIAS, regs, 9);
    } else {
